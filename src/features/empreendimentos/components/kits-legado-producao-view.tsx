...
export function KitsLegadoProducaoView({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {kits.map((k) => {
        const entregueErp = entreguePosErpPorKit[k.kit] ?? k.entreguePosErp;
        const produzidoHistoricoBase = Math.max(k.quantidadeProduzidaHistorico ?? 0, k.quantidadeEntregueHistorico);
        const pctHistorico = k.quantidadeContratada > 0 ? (produzidoHistoricoBase / k.quantidadeContratada) * 100 : 0;
        const pctErp = k.quantidadeContratada > 0 ? (k.quantidadeProduzidaPosErp / k.quantidadeContratada) * 100 : 0;
        const totalProduzidoPct = k.quantidadeContratada > 0 ? Math.min(100, (k.totalProduzido / k.quantidadeContratada) * 100) : 0;
        ...
        return (
          <div key={k.id} className="rounded-xl border border-border bg-card p-4">
            <div className="mb-3 flex items-baseline justify-between">
              ...
              <span className="text-lg font-bold tabular-nums text-primary">{Math.round(totalProduzidoPct)}% produzido</span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
              ...
              <div>
                <p className="text-[10px] text-muted-foreground">Produzido histórico</p>
                <p className="text-sm font-semibold tabular-nums text-muted-foreground">{produzidoHistoricoBase}</p>
              </div>
              <div>
              ...
              <div>
                <p className="text-[10px] text-muted-foreground">Entregue total</p>
                <p className="text-sm font-semibold tabular-nums text-primary">{k.quantidadeEntregueHistorico + entregueErp}</p>
              </div>
              <div>
